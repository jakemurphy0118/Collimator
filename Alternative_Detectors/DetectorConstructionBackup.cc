...
void DetectorConstruction::BuildSource(){
  ...
  // define a material from elements.   case 1: chemical molecule                                    
  density = 1.000*g/cm3;                                  
  G4Material* H2O = new G4Material(name="Water", density, ncomponents=2);                             H2O->AddElement(elH, natoms=2);                                                                    
  H2O->AddElement(elO, natoms=1);
  density = 0.200*g/cm3;
  G4Material* Aerog = new G4Material(name="Aerogel", density, ncomponents=3);
  //Silicon Dioxide (SiO2)
  ...
  G4Tubs* aeroDisk = new G4Tubs("aeroDiskSolid",0.0*cm,31.0*mm,2.0*cm,0,6.3);

  G4LogicalVolume* aeroDisk_log =
    new G4LogicalVolume(aeroDisk,Aerog, "Aerog log");

  double aerogelShift=2.2*cm;
  ...
  G4Material* ANTHR = man->FindOrBuildMaterial("G4_ANTHRACENE");

  //Scintillator
  G4double outerScintRadius = 31.0*mm;
  G4double innerScintRadius = 0.0*mm;
  G4double scintWidth = 0.25*cm;

  G4Tubs* scintDisk = new G4Tubs("scintDiskSolid",innerScintRadius,outerScintRadius,scintWidth,0,6.3);

  G4LogicalVolume* scintDisk_log =
    new G4LogicalVolume(scintDisk,ANTHR, "scint log");

  double scintDiskShift=0.0*cm;

  new G4PVPlacement(0,
		    G4ThreeVector(0,0,scintDiskShift),
		    scintDisk_log,
		    "Source",
		    LENALab_log,
		    false,
...
void DetectorConstruction::BuildSource(){
  ...
  double shiftPipe=halfPipeLength+halfGap;
  ...
  G4double innerRadius=4.5*cm;
  G4double outerRadius=5.0*cm;
  //Hollow source container
  G4Tubs* sourceTube = new G4Tubs("sourcePipeSolid",innerRadius,outerRadius,5.0*cm,3.14159,6.3);

  G4LogicalVolume* sourceTube_log =
    new G4LogicalVolume(sourceTube,steel1010A, "sourcePipe log");
  ...
  double aerogelShift=4.0*cm+0.5*cm;
  ...
  G4LogicalVolume* scintDisk_log =
    new G4LogicalVolume(scintDisk,Aerog, "scint log");

  double scintDiskShift=2.0*cm+0.5*cm;
