...
PhysicsList::PhysicsList() : G4VUserPhysicsList()//G4VModularPhysicsList()
{
  ...
  // Radioactive Decay is always defined
  raddecayList = new G4RadioactiveDecayPhysics();

  // EM physics
  //emName = G4String("emstandard");
  emName = G4String("emstandard_local");
  // Pass the Magnetic Field Here
  BField=5000;
  //BField=GetBFieldValue();

  emPhysicsList = new PhysListEmStandard(BField); //default to local list
  //  BField=emPhysicsList->GetBFieldValue();
  //emPhysicsList = new G4EmStandardPhysics(); //default to G4EmStandard list
}
...
G4double PhysicsList::GetBFieldValue()
{
  return BField;
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
...
void PhysicsList::ConstructProcess()
{
  ...
  AddTransportation();
  emPhysicsList->ConstructProcess();  //EM processes
  raddecayList->ConstructProcess();  //Radioactive decay process
  generalPhysicsList->ConstructProcess();  //only G4Decay process
  for(size_t i=0; i<hadronPhys.size(); i++) {
  ...
  //Register the oPS process
  //ConstructOPS();

}

/*void PhysicsList::ConstructOPS()
{
  G4ParticleDefinition* particle = G4Positron::Positron();
  G4ProcessManager* pmanager=particle->GetProcessManager();
  G4String string="annihil";
  G4oPSGeneration* opsDecay = new G4oPSGeneration(string);
  pmanager->AddDiscreteProcess(opsDecay);
  }*/
...
void PhysicsList::AddPhysicsList(const G4String& name)
{
  ...
  BField=GetBFieldValue();
  std::cout << "Add physics list: " << BField << std::endl;
  ...
  if (name == "emstandard") {
    ...
    emName = name;
    delete emPhysicsList;
    emPhysicsList = new PhysListEmStandard("standard", BField);
    //    emPhysicsList = new G4EmStandardPhysics("standard",BField);
    if (verboseLevel > 0) 
...
void PhysicsList::SetBField(G4double BFieldInput)
{
  BField=BFieldInput;
}
